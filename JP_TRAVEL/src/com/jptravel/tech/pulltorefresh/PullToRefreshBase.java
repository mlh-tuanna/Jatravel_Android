...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	// for load more
	public static final int MODE_RELEASE_TO_LOAD_MORE = 0x4;
	// ===========================================================
	// Fields
	// ===========================================================
	...
	/**
	 * Set OnLoadmoreListener for the widget
	 * 
	 * @param listener
	 *            -Listener to be used whwn the widget is set to Loadmore
	 */
	...
	public void setTimeLastRefresh(String time) {
		if (null != headerLayout) {
			headerLayout.setTime(time);
		}
	}
	...
	protected final PullToRefreshLayout getFooterLayout() {
		return footerLayout;
	}

	protected final PullToRefreshLayout getHeaderLayout() {
		return headerLayout;
	}
	...
	private void init(Context context, AttributeSet attrs) {
		try {
			...
			if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
				headerLayout = new PullToRefreshLayout(context, MODE_PULL_DOWN_TO_REFRESH, releaseLabel, pullLabel,
						refreshingLabel);
			...
			if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
				footerLayout = new PullToRefreshLayout(context, MODE_PULL_UP_TO_REFRESH, releaseLoad, pullLoadMore,
						refreshingLabel);
			...
			// If we're not using MODE_BOTH, then just set currentMode to
			// current
			// mode
			if (mode != MODE_BOTH) {
				currentMode = mode;
	...
	private boolean isReadyForPull() {
		...
		case MODE_PULL_UP_TO_REFRESH:
			return isReadyForLoadMore();
		case MODE_BOTH:
			return isReadyForLoadMore() || isReadyForPullDown();
		}
		return false;
	}
	...
	public static interface OnNonLastItemVisibleListener {

		public void onNonLastItemVisible();
